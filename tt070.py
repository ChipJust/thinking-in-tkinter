...
The EventType class is defined in the __init__.py of the tkinter module.
This class inherits from both str and enum.Enum and provides a __str__ method.
So, if we just call str on the type it will give us the name that goes with the
number.

Note that this example (and the previous one) is flawed in that we are binding to a button.
See this stack overflow question that is related:
https://stackoverflow.com/questions/28089035/tkinter-event-binding-remains-after-application-terminates
On my machine the terminal shows this error message from tk when I press Cancel, but not a traceback.
    invalid command name ".!frame.!button2"
        while executing
    "$w cget -state"
        (procedure "tk::ButtonDown" line 12)
        invoked from within
    "tk::ButtonDown .!frame.!button2"
...
class MyApp:
    def __init__(self, parent):
        ...
        #self.button2.bind("<ButtonRelease-1>", self.button2Click)
...
def report_event(event):     ### (5)
    ...
    ### (6)
    print ("{}, time= {}, type=({}, {}), widget={}, keysym={}".format(
        event,
        event.time,
        event.type,
        str(event.type),
